...
describe('Server', function () {
    ...
    it('should return default schema via promise', function (done) {
        var server = http_1.createServer(notFoundRequestListener);
        server.listen(SERVER_EXECUTOR_TESTS_PORT);
        var subServer = server_1.SubscriptionServer.create({
            schema: sinon.spy(function (connectionContext, params) {
    ...
    it('should return an error when invalid execute method provided', function (done) {
        var server = http_1.createServer(notFoundRequestListener);
        server.listen(SERVER_EXECUTOR_TESTS_PORT);
        server_1.SubscriptionServer.create({
            schema: schema,
        ...
        client.onConnected(function () {
            client.request({
                ...
                error: function (err) {
                    chai_1.expect(err.message).to.equal('GraphQL execute engine is not available');
                    client.client.close();
                    server.close();
                    done();
                },
...
describe('Client<->Server Flow', function () {
    it('should reconnect after manually closing the connection and then resubscribing', function (done) {
        var testServer = http_1.createServer(notFoundRequestListener);
    ...
    it('should close iteration over AsyncIterator when client unsubscribes', function () { return __awaiter(_this, void 0, void 0, function () {
        var testServer, createClientAndSubscribe, client1, client2;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    subscriptionAsyncIteratorSpy.reset();
                    resolveAsyncIteratorSpy.reset();
                    testServer = http_1.createServer(notFoundRequestListener);
                    testServer.listen(SERVER_EXECUTOR_TESTS_PORT);
                    server_1.SubscriptionServer.create({
                        schema: subscriptionsSchema,
                        execute: graphql_1.execute,
                            ...
                            setTimeout(function () {
                                client.request({
                                    ...
                                    next: function (s2Res) {
                                        ...
                                        chai_1.assert(s2Res.data !== null, 'unexpected null from 2nd ubscription result');
